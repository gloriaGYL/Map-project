...
def find_centroid(cluster):
    ...
    longitude = [restaurant_location(c)[1] for c in cluster]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    for f in feature_fns:
        each_predictor, each_rsquared = find_predictor(user,reviewed,f)
        pair_predictor_r.append([each_predictor,each_rsquared])
    return max(pair_predictor_r,key= lambda x: x[1])[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            each_pair = [r_name,user_rating(user,r_name)]
            pair.append(each_pair)
        else:
            each_pair = [r_name,predictor(r)]
            pair.append(each_pair)
    return dict(pair)
    # END Question 9
